...
function processGuess(champion) {
    ...
    const nameCol = createCol(champion.name, 'correct'); // Always show name as 'neutral' or correct style? Usually standard color or special. Loldle makes name square the rarity color, here we just show image.
    // Loldle usually just shows the square. Let's make it styled based on correctness of the guess itself (which is always 100% correct identity of the guess, but we want to show if it IS the target).
    ...
    if (isTarget) nameCol.classList.add('correct');
